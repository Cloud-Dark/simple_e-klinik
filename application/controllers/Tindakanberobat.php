...
class Tindakanberobat extends CI_Controller
{
   ...
   public function index()
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/logo-rs.jpg', 50, 5, 30);
        //$pdf->Image('', )
        // mencetak string 
        $pdf->Cell(45, 7, '', 0, 0, 'C');
        $pdf->Cell(230, 7, 'klinik3 WADAS', 0, 1, 'C');
  ...
  function cetakantrian($id){
        ...
        $tgl=date('Y/m/d');
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/latar-kartu.jpg',5,5,100,56);
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/latar-kartu.jpg',106,5,100,56);
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/logo-rs.jpg',10,9,10,10);
        $pdf->setFont('Arial','B',12);
        $pdf->Cell(90,5,'klinik3 WADAS',0,0,'C');
        $pdf->Cell(10,5,'',0,0,'C');
        ...
        $pdf->Cell(10,5,'',0,0,'C');
        $pdf->setFont('Arial','',7);
        $pdf->Cell(90,5,'- Kartu ini hanya bisa digunakan untuk berobat di klinik3 wadas.',0,1,'L');
   ...
   function rujukan_action(){
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/logo-rs.jpg', 22, 5, 30);
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(268, 7, 'klinik3 WADAS', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->Image('http://localhost/klinik3/assets/foto_profil/logo-rs.jpg', 4, 5, 30);
        //$pdf->Image('', )
        // mencetak string 
        $pdf->Cell(190, 7, 'klinik3 WADAS', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
